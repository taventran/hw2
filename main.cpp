   ...
   // students.print();
    ...
    while(getline(cfs, line)) {
        ...
        if (line.find("Add") != string::npos) {
            ...
            int howFar = index - beginIndex - 1; // Make sure to get it if it's multiple digits
        ...
        if (line.find("Sort") != string::npos) {
            ...
            int howFar = index - beginIndex - 1; // Get the sort condition
            string sortCond = line.substr(beginIndex+1, howFar);
            std::cout << sortCond << endl << endl;
